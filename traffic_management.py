...
FRAME_BUFFER_LEN = 90  # ~3 seconds buffer at 30 FPS
SPEED_THRESHOLD = 20     # pixels/frame to consider sudden speed change
COLLISION_THRESHOLD = 50 # pixels distance to consider collision
ACCIDENT_CLIP_DURATION = 90  # Frames (~3 seconds)
OUTPUT_FOLDER = "accident_clips"  # Where clips will be saved
...
class AccidentDetector:
    def __init__(self):
        ...
        self.frame_buffer = deque(maxlen=32)  # 32 frames for SlowFast
    ...
    def load_model(self):
        ...
        m.fc = nn.Linear(m.fc.in_features, 5)  # adjust for your classes
        return m
    ...
    def process_frame(self, frame):
        ...
        if len(self.frame_buffer) == 32:
            clip = torch.stack(list(self.frame_buffer)).unsqueeze(0).to(self.device)
...
frame_buffer = deque(maxlen=FRAME_BUFFER_LEN)  # For accident clips
...
while True:
    ...
    for r in results:
        for box in r.boxes:
            ...
            if ROI_TOP_LEFT[0] <= cx <= ROI_BOTTOM_RIGHT[0] and ROI_TOP_LEFT[1] <= cy <= ROI_BOTTOM_RIGHT[1]:
                ...
                cv2.putText(frame,str(cls),(x1,y1-5),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,255),2)
    ...
    if accident_status == "ACCIDENT_DETECTED" and len(frame_buffer) >= ACCIDENT_CLIP_DURATION:
        # Save clip
    ...
    # ---------------- DISPLAY INFO ----------------
    if accident_status:
        cv2.putText(frame,f"Status: {accident_status}",(20,120),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),3)
    cv2.line(frame,(VERTICAL_LINE_X,0),(VERTICAL_LINE_X,frame.shape[0]),(255,0,0),2)
    cv2.putText(frame,f"X = {VERTICAL_LINE_X}",(VERTICAL_LINE_X+10,30),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,0,0),2)
    for i, vehicles in lanes.items():
        cv2.putText(frame,f"Lane {i+1}: {len(vehicles)}",(20,160+i*25),cv2.FONT_HERSHEY_SIMPLEX,0.6,(255,255,255),2)
    cv2.putText(frame,f"Total Vehicles: {vehicle_count}",(20,40),cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,255),3)
    cv2.putText(frame,f"Queue Length: {vehicles_crossing_line}",(20,80),cv2.FONT_HERSHEY_SIMPLEX,1,(0,0,255),3)
